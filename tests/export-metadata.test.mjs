...
test('downloads show verified provenance and never date a mismatched or legacy file', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, 'myst.yml'), 'project:\n  downloads:\n    - file: exports/physics-of-music.pdf\n      title: Book\n');
    fs.writeFileSync(path.join(root, 'exports/physics-of-music.pdf'), 'pdf fixture');
    fs.writeFileSync(path.join(root, 'exports/metadata.json'), JSON.stringify({
      ...
      files: { 'physics-of-music.pdf': createHash('sha256').update('pdf fixture').digest('hex') },
    }));
    ...
    fs.writeFileSync(path.join(root, 'exports/physics-of-music.pdf'), 'different export');
    prepareDownloads(root);
    ...
    fs.unlinkSync(path.join(root, 'exports/physics-of-music.pdf'));
    prepareDownloads(root);
