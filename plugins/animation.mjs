 ...
 * container, and `css/custom.css` / `plugins/export.mjs` pick whichever of the
 ...
 * fetch: the fallback is simply the static SVG the matching
 * `scripts/figures/chNN_figures.py` function already generates, by
 * convention at `images/<id>.svg`. That mirrors `plugins/audio.mjs`'s bare-id
 * resolution, and for the same reason an iframe `src` needs it: MyST does not
 ...
 * `myst.yml` copies `animations/` there verbatim, alongside `audio/`.
 *
 ...
 * 2:1; other ratios need a matching rule in `css/custom.css`.
 *
 * @type {string}
 */
...
/**
 * Turns an `:aspect:` option into the CSS class `css/custom.css` keys on.
 *
