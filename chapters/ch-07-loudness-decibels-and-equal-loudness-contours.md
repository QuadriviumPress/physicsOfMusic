...
**Intensity** $I$ is the power per unit area carried by the wave, in watts per square metre. It is related to pressure by
...
A linear scale is hopeless here. If one millimetre on a graph represented the threshold of hearing, the threshold of pain would be a thousand kilometres away.
...
:alt: A vertical decibel scale from 0 to 130 dB with everyday and musical landmarks marked, and a second axis showing the corresponding intensities from ten to the minus twelve to one watt per square metre, with the region above 85 dB shaded as damaging.
...
The consequence for orchestration is sharp and slightly deflating. Sixty violins produce $10\log_{10}(60) = 18$ dB more than one. And since §7.5 will show that $10$ phons is a doubling of loudness, $18$ dB is under two doublings: **sixty violins sound less than four times as loud as one.** This is why orchestras have so many strings and so few trumpets — the section needs numbers merely to keep up.
...
The equal-loudness contours. Every point on one curve sounds equally loud. The curves are labelled in **phons**: by definition, a tone has a loudness level of $N$ phons if it sounds as loud as a $1$ kHz tone at $N$ dB SPL. At $1$ kHz, therefore, phons and dB SPL agree exactly; everywhere else they do not.
```
...
A sound that would be perfectly audible on its own can be rendered completely inaudible by another sound. This is **masking**, and [Chapter 6](#ch-the-ear) supplied the mechanism: a masker's travelling wave excites a region of the basilar membrane, and anything whose own region falls inside that excitation is lost in it.
...
The upward spread of masking has direct musical consequences. A loud bass line buries a quiet high part far more than the reverse, which is why arrangers give the melody to a high instrument and why a mix engineer who wants to hear the hi-hat turns the bass *down* rather than the hi-hat up.
...
- Loudness adds differently within and across critical bands. Spread the same energy across several bands and it sounds louder than if it is concentrated in one — which is why a chord is louder than a single note of the same total intensity, and why a full orchestration carries further than a doubled unison.

## Summary

- **Intensity goes as pressure squared.** This is why the decibel has two forms: $10\log_{10}$ for intensity and power, $20\log_{10}$ for pressure and amplitude. They describe the same sound.
...
(c) Ten clarinets for $10$ dB, which is one doubling of loudness. Getting a second doubling would take a hundred. This is why an ensemble cannot get much louder by recruiting, and why composers reach for instruments with intrinsically higher output — brass — rather than for more of the same.
:::
...
Therefore, $82$, $70$, and $54$ dB. At $50$ m outdoors the singer is at about the level of a quiet conversation — which is why outdoor performance essentially requires amplification, and indoor performance does not.
:::
...
(a) A band centred on $1$ kHz running from $f/\sqrt[6]{2}$ to $f\sqrt[6]{2}$:
...
It is worth noting that the coherent result cannot hold everywhere in space: constructive interference at one point must be paid for by destructive interference elsewhere ([Chapter 3](#ch-superposition)). The $6$ dB applies on the axis where the two arrive in step.
:::
...
(d) None of them explicitly — but (c) is the one that matters. A $3$ dB boost doubles the energy and is barely a noticeable change in loudness, which is why mixing moves are typically made in steps of $1$ to $3$ dB and why the smallest audible change in a mix context is often quoted as about $1$ dB.
:::

:::{exercise}
:label: ex-loudness-14

A hall has a background noise level of $30$ dB SPL. A solo flute produces $58$ dB at the back row. (a) What is the combined level? (b) What is the signal-to-noise ratio? (c) The audience arrives and the background rises to $42$ dB. Recompute both, and comment on what the audience has cost the flautist.
:::
...
Therefore, the audience has not made the hall measurably louder but has removed $12$ dB of the flautist's headroom. The quietest passages the player could previously shape are now within a few decibels of the noise floor and cease to be audible as music. This is why halls are built with such low background-noise specifications, and why a *pianissimo* is the first thing a noisy audience destroys.
:::
