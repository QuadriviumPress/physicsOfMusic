...
:alt: Three panels. Left, a harmonic line spectrum falling steadily, labelled source, the vocal folds. Centre, a decibel plot of a filter with three labelled formant peaks, labelled the vocal tract. Right, the resulting output spectrum with the filter's shape visible as an envelope over the harmonics.
...
This is the **opposite** of a wind instrument, where the bore controls the reed. It is worth noticing that the voice is therefore acoustically more like a string instrument — source sets pitch, resonator shapes tone — than like the wind instrument it superficially resembles.
...
Here is the problem. An orchestra playing fortissimo produces around $100$ dB in the hall. A single unamplified human voice produces perhaps $90$ dB at a metre, and less by the time it reaches the audience. Adding one voice to a hundred instruments should be inaudible, by [Chapter 7](#ch-loudness)'s addition rule.
...
- **The voice is a source–filter system.** The vocal folds are the source, producing a harmonic series at the pitch; the vocal tract is the filter, whose resonances — **formants** — shape it. The two are essentially independent, which is why pitch and vowel can vary separately.
...
- **Fundamental frequency rises with fold tension and falls with fold mass**, and rises slightly with breath pressure — which is why loud singing tends to go sharp.
...
- **Formants do not move when the pitch does**, which is why a vowel survives across a singer's range — up to a point.
...
Therefore the singer trades intelligibility for power, and above a certain pitch she has no real choice: the alternative is to be inaudible. This is why sung text at the top of the soprano range is conventionally supported by surtitles.
:::
...
Vocal folds are modelled as a mass on a spring. A singer raises the pitch from $220$ Hz to $440$ Hz. (a) If the effective mass is unchanged, by what factor does the tension rise? (b) In practice the folds also thin as they stretch. How does that affect the answer?
:::
...
Therefore vibrato causes the output amplitude of each harmonic to fluctuate as it moves across the filter's slopes — which is why vibrato produces amplitude modulation as well as frequency modulation, and why it makes a voice easier to pick out of a texture.
:::
...
[Chapter 7](#ch-loudness) established that masking spreads **upward** in frequency far more than downward, because a low-frequency travelling wave must traverse the whole basilar membrane to reach its own place and excites the high-frequency region on the way ([Chapter 6](#ch-the-ear)).
...
Therefore the region where the voice is least masked is the region where the singer's formant puts its energy, and it is also where the ear is most sensitive. The three effects reinforce each other, which is why the solution works as well as it does.
:::
...
It is heard because the comparison is not made broadband. In the narrow $3$ kHz band where the singer's formant concentrates the voice's output and where the orchestra has little, the voice is *above* the orchestra. The ear analyses by critical band ([Chapter 7](#ch-loudness)), so what matters is the signal-to-masker ratio within a band, not the total.
...
In a **violin** ([Chapter 10](#ch-string-instruments)), the string sets the pitch and the body merely responds. The body could be replaced with a different one and the same notes would sound, differently coloured.
...
3. **Moving formants.** Formants do not jump between target values; they glide, and the glides between vowels — the *transitions* — carry much of the information that identifies neighbouring consonants. A synthesizer that hits formant targets and holds them produces separately correct vowels and unnatural speech.
