...
Every instrument so far has had a harmonic spectrum. Strings and air columns are one-dimensional, and their boundary conditions force their modes into exact whole-number ratios — which is why they have clear pitches, why their tones blend, and why [Chapter 9](#ch-scales-and-tuning)'s tuning systems are built out of small integers.
...
This is why percussion divides so sharply into instruments of **definite** and **indefinite** pitch, and why the definite ones — timpani, marimba, tuned bells — are the result of considerable ingenuity. Each is an object that has been *made* to have a pitch, by moving its modes into place. This chapter is largely about how.
...
:alt: Six panels showing the first six modes of a circular membrane as red and blue regions with black nodal lines, labelled (0,1) at 1.00 f1, (1,1) at 1.59, (2,1) at 2.14, (0,2) at 2.30, (3,1) at 2.65 and (1,2) at 2.92.

The first six modes of an ideal circular membrane. Each is labelled $(m, n)$ with $m$ nodal **diameters** and $n{-}1$ nodal **circles** inside the rim. The frequency ratios — $1.00$, $1.59$, $2.14$, $2.30$, $2.65$, $2.92$ — are not whole numbers, are not close to whole numbers, and are not multiples of anything.
```
...
Given $200, 318, 428, 460, 530$ Hz it finds nothing. There is no fundamental that explains this set, and the pattern-matching mechanism has no answer to return. The listener hears a sound with a *centre of gravity* — they can say it is high or low, and can tell one drum from another — but not a note. They cannot sing it back, and it does not participate in harmony.
...
The fundamental has its maximum displacement — its antinode — at the centre of the bar. The second mode has a *node* there. Removing material from the centre therefore weakens the bar where the fundamental is bending hardest and where the second mode is barely bending at all, so it lowers the fundamental far more than it lowers the second mode.
...
| Vibraphone | $4f_1$ | Mellow, sustained (aluminium, long decay) |
...
This has a practical consequence percussionists use constantly. Striking at the centre excites only the modes with no nodal diameter — $(0,1)$, $(0,2)$, and so on — because every mode with $m \ge 1$ has a node there. Striking near the rim excites the $(m,1)$ family strongly.
...
:alt: Four square panels showing black nodal-line patterns of increasing complexity, from a simple diagonal cross to an intricate lattice with a star at the centre.
...
This is why a crash cymbal *blooms*: it does not simply start loud and decay, it starts with a limited spectrum and then fills out, brightening as it goes. It is one of the very few places in this book where superposition genuinely fails, and it is audible.
...
:alt: Five labelled vertical bars at frequency ratios 0.5, 1.0, 1.2, 1.5 and 2.0, named hum, prime, tierce, quint and nominal, and annotated as octave below, the strike note, minor third, fifth and octave above.

The five tuned partials of a church bell. Four are placed on consonant intervals; the **tierce** is deliberately a *minor* third, which is why bells sound solemn rather than bright, and why a major-third bell — which founders can now make — sounds subtly wrong to most listeners.
```
...
Percussion instruments with definite pitch are made so by one of three strategies, and it is worth naming them together because the whole chapter is instances of them.
...
**Suppress the modes that do not fit.** Strike a timpano off-centre to avoid the $(0,1)$ piston mode. Support a bar at the fundamental's nodes.
...
It is worth being explicit about what "pitch" means for these instruments, because it is not quite what it means for a violin.
...
- **Undercutting a bar** removes material at the centre, where the fundamental has an antinode and the second mode a node, lowering the first far more than the second. Xylophone bars are tuned to $3f_1$, marimba bars to $4f_1$.
...
3. Explain why removing wood from the centre of a marimba bar lowers the fundamental much more than the second mode.

4. Explain why a marimba bar is supported at about $22\%$ of its length from each end rather than at the ends themselves.

5. A timpanist strikes about a quarter of the way in from the rim rather than at the centre. Give two reasons.
...
Therefore the listener hears about $73$ Hz — D2 — which is an octave below the lowest mode present and which the instrument radiates essentially nothing at. The fourth and fifth of these are a little flat of exact harmonics, which is why a timpano's pitch, while definite, is less crisply defined than a string's.
:::
...
Therefore about $4.1$ kN/m of tension, and a $26\%$ increase for a whole tone — which is why a timpano's pedal has to move a considerable distance and why the instrument's range is limited to about a fifth.
:::
...
(c) Both are exact harmonics and therefore consonant. $4{:}1$ is the simpler ratio and blends more completely into the fundamental, which is why the marimba sounds warm and unified while the xylophone sounds like a fundamental with a distinct twelfth on top.
:::
...
A timpanist strikes a head at its exact centre. (a) Which modes are excited? (b) Which are not? (c) Describe the resulting sound and explain why timpanists never do this.
:::
...
(a) Only the modes with **no nodal diameter**: $(0,1)$, $(0,2)$, $(0,3)$, … These are the circularly symmetric ones, which move at the centre.

(b) Every mode with $m \ge 1$ — $(1,1)$, $(2,1)$, $(3,1)$, and so on — because all of them have a nodal diameter through the centre.

(c) The $(m,1)$ family is precisely the near-harmonic set that gives the timpano its pitch, so striking at the centre removes it entirely. What remains is the $(0,n)$ family, whose ratios are $1 : 2.30 : 3.60$ — thoroughly inharmonic — plus the $(0,1)$ piston mode, which radiates very loudly and decays fast.

Therefore a centre strike produces a loud, dull, pitchless thud. Timpanists strike about a quarter of the way in precisely to avoid it, which puts the mallet near a node of $(0,1)$ and at a strong antinode of $(1,1)$.
:::

:::{exercise}
:label: ex-percussion-9

Show that supporting a free-free bar at $22.4\%$ of its length from each end places the supports at nodes of the fundamental, and explain what would happen if they were placed at the centre instead.
:::
...
Supporting at the centre would put the support at an **antinode** of the fundamental — the point of maximum motion. The support would absorb energy at the greatest possible rate, and the fundamental would be heavily damped. What would survive is the second mode, which has a node at the centre.
...
A listener's judgement follows what they are used to hearing, and both judgements are correct about different things: the unfamiliar listener is hearing a real acoustic mismatch between the intervals and their *expectations*, which were formed on harmonic instruments.
